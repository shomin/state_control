...
wd = zeros(N,2);
for i = 1:N,
    tx = text(x(i),y(i),fix_title_string(str{i}),'HorizontalAlignment','center','VerticalAlign','middle','FontSize', fontsize);
    sz = get(tx, 'Extent');
    wy = sz(4);
    wx = max(2/3*sz(3), wy); 
...
N = length(str);
for i = 1:N,
    tx = text(x(i),y(i),fix_title_string(str{i}),'HorizontalAlignment','center','VerticalAlign','middle');
    sz = get(tx, 'Extent');
