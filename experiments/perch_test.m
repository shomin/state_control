...
states = [...
    ...
    create_state(@xyz_vel, gains, bottom, 'speed', speed,'accelrate', accel,@n_plus,1, b_step{:});...
    create_state(hover{:});...
    
    
    %go to top with man_speed and man_accel
    create_state(@xyz_vel, gains, top, 'speed', man_speed,'accelrate', man_accel,@n_plus,1,...
        @orientation,'z_est','over',z_mid,@n_plus,1,...
        b_step{:});...

    create_state(@xyz_vel, gains, target_top, 'speed', man_speed,'theta_cmd',theta_cmd,'thrust_cmd',thrust_cmd,@n_plus,2,...
    ...
    create_state(@xyz_vel, gains, target_top, 'speed', man_speed,'theta_cmd',0,'thrust_cmd',60,@n_plus,1,...
        @ec_timer,.3,@n_plus,1,...
    ...
    create_state(@xyz_vel, gains, land, 'speed', speed,'accelrate', accel,@n_plus,1, b_step{:});...
    create_state(hover{:},...
...
%%
run_states(tester,curr_state   , states, @sim_update,.001,'~/Desktop/perch_sim',h);
